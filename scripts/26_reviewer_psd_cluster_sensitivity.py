...
def main() -> None:
    ...
    ap.add_argument("--tf_rep", choices=["cwt", "stft"], default="cwt")
    ap.add_argument("--device", default="auto")
    ap.add_argument("--method", default="pi_fsl")
    ap.add_argument("--variant", default="full")
    ap.add_argument("--seeds", default="1,2,3")
    ap.add_argument("--normalization", default="per_window")
    ap.add_argument("--clusters", default="2,3,4,5")
    ap.add_argument("--target_ratios", default="0.30,0.40,0.50")
    ap.add_argument("--resume", action="store_true")
    ap.add_argument("--eval_artifacts_dir", default="outputs/eval_artifacts")
    args = ap.parse_args()

    root = repo_root()
    cfg = load_yaml(root / args.paths)

    out_jsonl = root / "artifacts" / "results" / "jsonl" / "psd_cluster_sensitivity.jsonl"
    out_jsonl.parent.mkdir(parents=True, exist_ok=True)
    done = load_done_pairs(out_jsonl) if args.resume else set()

    eval_dir = (root / args.eval_artifacts_dir).resolve()
        ...
        for seed in seeds:
            for k in clusters:
                for r in target_ratios:
                    scenario = f"{args.exp}_{src_domain_arg}_to_{tgt}__psdK{k}__r{r:.2f}"
                    ...
                    cmd = [
                        ...
                        "--psd_guided_balance",
                        "--psd_n_clusters", str(k),
                        "--psd_target_ratio", f"{r:.2f}",
                    ]
