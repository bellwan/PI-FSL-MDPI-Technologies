...
def _repo_root() -> Path:
    return Path(__file__).resolve().parents[1]  # repo root (one above /scripts)
...
def main() -> None:
    ...
    ap.add_argument("--psd_guided_balance", action="store_true")
    ap.add_argument("--out_jsonl", default="artifacts/results/jsonl/head_ablation_bosch.jsonl")
    ap.add_argument("--methods", default="pi_fsl,relationnet,protonet,matchingnet",
                    help="Comma-separated methods to run.")
    ap.add_argument("--variant_pi_fsl", default="full")
        ...
        for tgt in tgt_domains:
            ...
            for seed in seeds:
                for method in methods:
                    ...
                    if args.psd_guided_balance:
                        cmd += ["--psd_guided_balance"]

                    if method == "pi_fsl":
                        cmd += ["--variant", args.variant_pi_fsl]
                        if args.variant_pi_fsl not in ("wo_fsl", "wo_fsl_ft"):
