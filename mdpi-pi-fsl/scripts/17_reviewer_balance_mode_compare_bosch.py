...
def main() -> None:
    ...
    ap.add_argument("--out_jsonl", default="artifacts/results/jsonl/balance_mode_compare_bosch.jsonl")
    args = ap.parse_args()
    ...
    modes = [
        ("none", []),
        ("psd", ["--psd_guided_balance", "--notes", "psd_guided_balance"]),
        ("random", ["--balance_mode", "random", "--notes", "random_class_balance"]),
    ]

    for tgt in tgt_domains:
        for seed in seeds:
            for mode_name, mode_args in modes:
                scenario = f"{args.exp}_{src_domain_arg}_to_{tgt}__bal_{mode_name}"
