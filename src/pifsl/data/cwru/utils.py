...
def make_time_frequency_image_STFT(dataset_name, 
    ...
    # Image Plotting (use explicit fig/ax)
    fig, ax = plt.subplots()
    c = ax.pcolormesh(time, frequency, magnitude, shading='gouraud')
    ax.axis('off')
...
def make_time_frequency_image_WT(dataset_name,
    ...
    # Image Plotting (explicit fig/ax)
    t = np.linspace(0, sampling_period, sampling_length, endpoint=False)
    fig, ax = plt.subplots()
    c = ax.contourf(t, frequencies, amplitude, cmap='jet')
    ax.axis('off')
